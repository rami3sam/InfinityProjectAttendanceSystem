...
def drawOnFrame(i,frame,boxes,student_id):
    ...
    logging.info('face {} : coordinates {} , {}'.format(i,startpoint,endpoint))
...
def calculateEmbeddingsErrors(resnet,aligned,all_embeddings):
    ...
    for key in all_embeddings:
        ...
        logging.info('{} distances are: {}'.format(key ,dist_arr))
        min_dist = min(dist_arr)  
        dist_dict[min_dist] = key
        logging.info('{} minimum  distance is : {}'.format(key , min_dist))
    min_dist = min(dist_dict.keys())
    nearest_student = dist_dict.get(min_dist)
    logging.info(' student is likely to be: {} '.format(nearest_student).center(80,'*'))
    logging.info(' least distance is: {} '.format(min_dist).center(80,'*'))
    return min_dist,nearest_student
